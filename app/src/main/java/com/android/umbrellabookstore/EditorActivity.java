...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    /** EditText field to enter the product's qauntity */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBookQuantityEditText = findViewById(R.id.edit_book_quantiy);
    ...
    private void saveBook() {
       ...
       //check if the values are entered for the new book
        ...
        values.put(UmbrellaBookStoreEntry.COLUMN_PRODUCT_NAME, productName);
        if(!TextUtils.isEmpty(bookPrice)) {
            values.put(UmbrellaBookStoreEntry.COLUMN_BOOK_PRICE, bookPrice);
        } else{
            values.put(UmbrellaBookStoreEntry.COLUMN_BOOK_PRICE, bookPriceDefaultValue);

        }
        if(!TextUtils.isEmpty(bookQuantity)) {
            values.put(UmbrellaBookStoreEntry.COLUMN_BOOK_QUANTITY, Integer.parseInt(bookQuantity));
        }else{
            values.put(UmbrellaBookStoreEntry.COLUMN_BOOK_QUANTITY, bookQuantityDefaultValue);
        }
        values.put(UmbrellaBookStoreEntry.COLUMN_BOOK_SUPPLIER_NAME, supplierName);
        values.put(UmbrellaBookStoreEntry.COLUMN_BOOK_SUPPLIER_PHONE_NUMBER, supplierPhone);
    ...
    if (currentBookUri == null) {
        ...
        if (newRowId == -1) {
            ...
            Toast.makeText(this, R.string.editor_save_book_failed, Toast.LENGTH_SHORT).show();
        } else {
            // Otherwise, the insertion was successful and we can display a toast with the row ID.
            Toast.makeText(this, R.string.editor_save_book_successful, Toast.LENGTH_SHORT).show();
        }
    }
    }
    ...
    public Loader<Cursor> onCreateLoader(int i, Bundle bundle) {
        ...
        String[] projection = {
                ...
                UmbrellaBookStoreEntry.COLUMN_BOOK_SUPPLIER_NAME,
                UmbrellaBookStoreEntry.COLUMN_BOOK_SUPPLIER_PHONE_NUMBER,
        };
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        if (cursor.moveToFirst()) {
            ...
            int productNameIndex = cursor.getColumnIndex(UmbrellaBookStoreEntry.COLUMN_PRODUCT_NAME);
            int bookPriceIndex = cursor.getColumnIndex(UmbrellaBookStoreEntry.COLUMN_BOOK_PRICE);
            int bookQuantityIndex = cursor.getColumnIndex(UmbrellaBookStoreEntry.COLUMN_BOOK_QUANTITY);
            int supplierNameIndex = cursor.getColumnIndex(UmbrellaBookStoreEntry.COLUMN_BOOK_SUPPLIER_NAME);
            int supplierPhoneNumberIndex = cursor.getColumnIndex(UmbrellaBookStoreEntry.COLUMN_BOOK_SUPPLIER_PHONE_NUMBER);
